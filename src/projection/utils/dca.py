import sys, os, glob
from coo_matrix_nocheck import CooMatrix
import numpy as np
...
def remove_dummies(ttree, orig_target_length):
	...
	while len(intersection)>0:
		for x in xrange(orig_target_length,len(ttree)): #for all dummies
			for y in xrange(orig_target_length): #for all non-dummies
				if ttree[y]==(x+1):
					ttree[y]=ttree[x]
		intersection=[i for i in range(orig_target_length+1,len(ttree)+1) if i in ttree[:orig_target_length]]
...
def one2one(align_ts, align_st, stree, ttree):
	...
	for x in xrange(1, len(align_ts.keys())+1):
		t=align_ts[x]
...
def unaligned_source(align_ts, align_st, ttree):
	...
	for i in xrange(1,len(align_st)+1):
		s=align_st[i]
		if len(s)==0:
...
def many2one(align_ts, align_st, ttree, stree, depths):
	...
	for i in xrange(1,len(align_ts)+1):
		t=align_ts[i][:]
		if len(t)>1:
			align_ts[i]=[t[0]]
...
def getAlignments(align_data, slength, tlength):
	...
	align_ts={(x+1):[] for x in xrange(tlength)}
	align_st={(x+1):[] for x in xrange(slength)}
	for a in align_data:
		st_pair=a.split('-')
...
def getAlignmentsSparse(A_sparse):
	...
	#2) source node --> [target nodes]
	#the indices of nodes in adj lists are same as the id from the conll files!
	align_ts={(x+1):[] for x in xrange(A_sparse.shape[1]-1)}#remove pseudo-root
	align_st={(x+1):[] for x in xrange(A_sparse.shape[0]-1)}
	for a in xrange(len(A_sparse.row)):
		if A_sparse.row[a]>0:
...
def makeSourceTreeSparse(S_sparse):
	...
	for x in xrange(len(S_sparse.row)):
...
def getDepths(stree):
	...
	depths[rootindex]=0
	for i in xrange(len(stree)): #putting children of root onto queue q
		if stree[i]==(rootindex+1):
			q.append(i)
	while len(q)>0:
		curr=q.pop(0)
		depths[curr]=depths[stree[curr]-1]+1
		for i in xrange(len(stree)):
			if stree[i]==(curr+1):
				q.append(i)
	return depths
...
def getMatchNumber(ttree, gold_ttree):
	matches=[1.0 if ttree[x]==gold_ttree[x] else 0 for x in xrange(len(gold_ttree))]
	return sum(matches)		

def readAllSentsToMemory(sourcefile,targetfile):
	sfile=open(sourcefile,'r'); slines=sfile.readlines(); sfile.close()
	tfile=open(targetfile,'r'); tlines=tfile.readlines(); tfile.close()
...
def readFiles(sourcefile, targetfile, alignmentfile, salignmentfile):
	...
	afile=open(alignmentfile, 'r'); alines=afile.readlines(); afile.close()
	safile=open(salignmentfile, 'r'); salines=safile.readlines(); safile.close()
	
	a_index=0
	sum_tp=0; sum_total=0; sum_score=0
	while a_index<len(alines):
		#print '\r',a_index,
		salign_data=salines[a_index].strip().split('\t')
		align_data=alines[a_index].strip().split()[::2]; a_index+=1
...
def testSparseMatrices(sourcefile, targetfile, alignmentfile, salignmentfile):
	...
	while a_index<len(alines):
		...
		ttree=[-10]*(len(t_sent))
		for x in xrange(1,len(T_matrix)):
			for y in xrange(len(T_matrix[x])):
				if T_matrix[x][y] ==1 :
					ttree[x-1]=y
		stree_orig, ttree_orig=projectArrays(t_sent,s_sent,align_data, salign_data)
		for x in xrange(len(ttree)):
...
def printScores(outfilebasename, scores):
	...
	for score in sorted(scores):
		latexfile.write(score[0]+'&'+score[1]+'&'+score[2]+'&'+str(round(score[3],3))+'&'+str(round(score[4],3))+'\\\\\n' )
...
def iterateThroughDir_Bible():
	...
	if args.sourcefile is None:
		sources=['ar','bg','cs','da','de','en','es','eu','fa','fi','fr','he','hi','hr','id','it','no','pl','pt','sl','sv']
		#sources=['de','en','es','fr']
	else:

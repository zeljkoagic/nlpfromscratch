    ...
    for l1 in `cat /home/bplank/preprocess-holy-data/languages/trg-src-${corpus}.txt`; do
        for l2 in `cat /home/bplank/preprocess-holy-data/languages/trg-src-${corpus}.txt`; do
            if [ "$l1" != "$l2" ]; then
                ...
                echo "$HOME/tools/walign/fast_align/fast_align -v -r -i /home/bplank/preprocess-holy-data/data/walign/${l1}-${l2}.${corpus}.spairs 1> $HOME/data/walign/${l1}-${l2}.${corpus}.wal 2> $HOME/data/logs/${l1}-${l2}.${corpus}.log" >> $HOME/run/commands_walign_${corpus}.txt
            fi
        done
    done
done

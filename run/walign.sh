...
for l1 in `cat $HOME/data/lists/all_languages.txt`; do
    for l2 in `cat $HOME/data/lists/all_languages.txt`; do
        if [ "$l1" != "$l2" ]; then
            # extract sentence pairs (does lowercasing too)
            # currently supports only fast_align!
            echo "python $HOME/src/helper/extract_sentence_pairs.py $HOME/data/raw/${l1}.txt $HOME/data/raw/${l2}.txt $HOME/data/salign/${l1}-${l2}.sal 1> $HOME/data/walign/${l1}-${l2}.spairs 2> $HOME/data/logs/${l1}-${l2}.spairs.log" >> $HOME/run/commands_spair.txt
